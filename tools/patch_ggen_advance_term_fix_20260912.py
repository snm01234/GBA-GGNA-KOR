"""Correct leftover JP calques and proper-noun drift, then emit a main-TIP POC.

Japanese sources:
- カトンボみたいっ (stored as カトンペ via uncorrected kana) → 잔챙이처럼
- キャスバル leftover 카스발 → 캐스발
- クスフィアス → 크시피어스
- ボップ・ミサイル → 밥 미사일
- グロムリン leftover 그로무린 → 그롬린
- ロームフェラ leftover 롬펠라/로멜펠러 → 롬펠러
- 痛撃/痛撃率 → 치명/치명률
- てぇいっ phonetic 테에잇 → 쏴라아
- ぬう/ぬうっ/ぬ thinking grunts → 으음/으으음/음
"""
...
BATCH_ID = "term-fix-katonbo-casval-xiphias-20260912"
IDENTITY_KEY = "term_fix_20260912_sha256"
BATCH_KEY = "term_fix_20260912"
OUT = ROOT / "outputs" / "20260912_term_fix"
WORK = OUT / "ggen_term_fix_20260912.gba"
SNAPSHOT = ROOT / "analysis" / "ggen_advance_translation_merged_20260912_term_fix.json"
...
CAVE_START = 0x01135370
CAVE_END = 0x01230000
MAP_BANK = (0x00F00000, 0x00FC0000)
NOTES = (
    "용어 교정: カトンボ→잔챙이, キャスバル 카스발→캐스발, クスフィアス→크시피어스, "
    "ボップミサイル→밥 미사일, グロムリン 잔재, ロームフェラ→롬펠러, 痛撃→치명, "
    "테에잇→쏴라아, ぬう→으음"
)
...
def encode_u32_payload(
    ...
    *,
    ...
    from patch_ggen_advance_dialogue_idcmd_fit_20260909 import live_char_tokens
...
def main() -> None:
    ...
    gate(targets, "no term-fix rows found")
    ...
    extra_hangul = hangul_chars("치명잔챙이처럼쏴밥러으음카톤페")
    ...
    proofs = {
        "katonbo": str(by_id["GGA-SCENARIO-002042F8"].get("translation_ko") or ""),
        "xiphias": live_has(by_id["GGA-TEXT-0017A6FA"], "크시피어스"),
        "xiphias_dyn": live_has(by_id["GGA-DYNAMIC-001F1DA7"], "크시피어스"),
        "bop": live_has(by_id["GGA-TEXT-00179F0E"], "밥 미사일"),
        "gromlin_id": live_has(by_id["GGA-TEXT-0017BAFE"], "그롬린"),
        "casval": str(by_id["GGA-MAPSCRIPT-00FA37AB"].get("translation_ko") or ""),
        "romefeller_map": str(by_id["GGA-MAPSCRIPT-00F96DF6"].get("translation_ko") or ""),
        "crit_effect": live_has(by_id["GGA-TEXT-0017B4EA"], "치명"),
    }
    gate("잔챙이처럼" in proofs["katonbo"] and "이리저리" in proofs["katonbo"], ascii(proofs["katonbo"]))
    gate("크시피어스" in proofs["xiphias"] and "크스피어스" not in proofs["xiphias"], ascii(proofs["xiphias"]))
    gate("크시피어스" in proofs["xiphias_dyn"] and "크스피어스" not in proofs["xiphias_dyn"], ascii(proofs["xiphias_dyn"]))
    gate("밥 미사일" in proofs["bop"] and "펩 미사일" not in proofs["bop"], ascii(proofs["bop"]))
    gate("그롬린" in proofs["gromlin_id"] and "그로무린" not in proofs["gromlin_id"], ascii(proofs["gromlin_id"]))
    gate("캐스발" in proofs["casval"] and "카스발" not in proofs["casval"], ascii(proofs["casval"]))
    gate("롬펠러" in proofs["romefeller_map"] and "로멜펠러" not in proofs["romefeller_map"], ascii(proofs["romefeller_map"]))
    gate("치명" in proofs["crit_effect"] and "통격" not in proofs["crit_effect"], ascii(proofs["crit_effect"]))
    ...
    shutil.copy2(MAIN_SAV, OUT / "ggen_term_fix_20260912.sav")
    report = {
        "schema_version": 1,
        "kind": "ggen_advance_term_fix_20260912",
        "result": "PASS",
        "batch_id": BATCH_ID,
        ...
        "verification": {
            ...
            "katonbo_is_small_fry": True,
            "xiphias_unified": True,
            "gromlin_residual_cleared": True,
            "romefeller_unified": True,
            "tsuugeki_to_critical": True,
            "runtime_emulator": "not run; static ROM verification only",
            "changed_bytes": len(changed),
        },
    }
    (OUT / "manifest.json").write_text(json.dumps(report, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    (ROOT / "analysis" / "ggen_advance_term_fix_candidate_20260912.json").write_text(
