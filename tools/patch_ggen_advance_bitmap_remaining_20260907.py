#!/usr/bin/env python3
"""Apply remaining bitmap-closed 8x16 lines to the sheet and current-main ROM."""
...
BATCH_ID = "bitmap-remaining-20260907"
KO_MAP = ROOT / "analysis" / "ggen_advance_8x16_bitmap_remaining_ko_20260907.json"
SNAPSHOT = ROOT / "analysis" / "ggen_advance_translation_merged_20260907_bitmap_remaining.json"
OUT_DIR = ROOT / "outputs" / "20260907_ggen_advance_bitmap_remaining"
OUTPUT = OUT_DIR / "ggen_advance_bitmap_remaining_candidate_20260907.gba"
OUT_SAV = OUT_DIR / "ggen_advance_bitmap_remaining_candidate_20260907.sav"
MAIN_SAV = ROOT / "SD Gundam GGeneration Advance (Korean).sav"
REPORT = ROOT / "analysis" / "ggen_advance_bitmap_remaining_candidate_20260907.json"
MANIFEST = OUT_DIR / "manifest.json"
CAVE_START = 0x012B1100
CAVE_END = 0x012C0000
EXTRA_CATS = {
    "series_title",
...
def main() -> int:
    ...
    gate(all(value == 0 for value in current[CAVE_START:CAVE_END]), "bitmap-remaining cave is not zero-filled")
    ...
    for row in merged["records"]:
        ...
        if category == "id_command_name":
            ...
            if korean is None:
                ...
                continue
            jobs.append({"row": row, "source": decoded, "korean": korean, "notes": "8x16 비트맵 leftover 잔여 ID커맨드"})
            continue
    ...
    for job in jobs:
        ...
        if key in written:
            ...
            gate(end <= CAVE_END, "bitmap-remaining cave overflow")
    ...
    identity["bitmap_remaining_batch_identity_sha256"] = new_identity
    ...
    merged["bitmap_remaining_batch_20260907"] = {
        "batch_id": BATCH_ID,
