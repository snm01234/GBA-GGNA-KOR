#!/usr/bin/env python3
"""Apply user-confirmed base names to the remaining unit-name candidates.

The two remaining owner-proven unit-name candidates are updated across every
included occurrence.  The Hyaku Shiki variant keeps the locally supported
``<MBL>`` suffix; the Archangel row is intentionally rendered as the
user-confirmed base ``아크엔젤`` while the globally ambiguous source slot
0x021D remains documented in the audit note instead of being guessed.
"""
...
DEFAULT_REPORT = ROOT / "analysis" / "ggen_advance_remaining_unit_name_bases_20260830.json"
BATCH_ID = "user-confirmed-unit-name-bases-20260830"
TARGETS = {
    "百式<MB<00EC>>": {
        "translation_ko": "백식<MBL>",
        "note": "user-confirmed base-name mapping: 百式 → 백식; retained the local <MBL> variant suffix",
    },
    "アークエンジェル<021D>": {
        "translation_ko": "아크엔젤",
        "note": "user-confirmed base-name mapping: アークエンジェル → 아크엔젤; source slot 0x021D was omitted from the display pending global resolution",
    },
}
...
def update_summary(payload: dict[str, Any], correction_identity: str) -> None:
    ...
    merged_counts = dict(sorted(merged_statuses.items()))
    summary.update(
        {
            "merged_translation_status_counts": merged_counts,
            ...
            "remaining_unit_name_base_correction_records": 5,
        }
    )
    payload["merged_translation_status_counts"] = merged_counts
...
def main(argv: list[str] | None = None) -> int:
    ...
    target_rows = [
        ...
        and leading_reserved_body(str(row.get("source_text") or "")) in TARGETS
        and str(row.get("translation_status")) == "pending"
    ]
    counts = Counter(leading_reserved_body(str(row.get("source_text") or "")) for row in target_rows)
    check(counts == Counter({"百式<MB<00EC>>": 4, "アークエンジェル<021D>": 1}), f"remaining unit-name target set drift: {dict(counts)}")
    check(len(target_rows) == 5, f"expected five remaining unit-name rows, got {len(target_rows)}")
    ...
    for row in sorted(target_rows, key=lambda item: str(item["record_id"])):
        source_body = leading_reserved_body(str(row.get("source_text") or ""))
        target = TARGETS[source_body]
        before = {
            "translation_ko": row.get("translation_ko", ""),
        ...
        row["translation_ko"] = target["translation_ko"]
        ...
        row["translator_notes"] = f"{note}; {target['note']}".lstrip("; ")
    ...
    identity["remaining_unit_name_base_correction_identity_sha256"] = correction_identity
    ...
    payload["remaining_unit_name_base_correction"] = {
        "batch_id": BATCH_ID,
    ...
    pending_table = [row for row in table_rows if row.get("translation_status") == "pending"]
    report = {
        "schema_version": 1,
        "kind": "ggen_advance_remaining_unit_name_bases",
        "batch_id": BATCH_ID,
        ...
        "post_update_counts": {
            "unit_name_table_records": len(table_rows),
            "unit_name_table_pending_records": len(pending_table),
            "unit_name_table_pending_unique_sources": sorted(
                {
                    leading_reserved_body(str(row.get("source_text") or ""))
                    for row in pending_table
                }
            ),
            "unit_name_pending_records": sum(
                row.get("semantic_category") == "unit_name" and row.get("scope_status") == "included" and row.get("translation_status") == "pending"
                for row in records
            ),
    ...
    print(json.dumps({"result": "PASS", **payload["remaining_unit_name_base_correction"], "report": str(args.report)}, ensure_ascii=False, indent=2))
    return 0
