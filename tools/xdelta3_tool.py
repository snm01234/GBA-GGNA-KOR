"""Locate or fetch a pinned xdelta3 binary for G Generation Advance deltas.
...
from ggen_advance_project_paths import ADVANCE_ROOT, ORIGINAL_ROM_SIZE, MAIN_TIP_ROM_SIZE, advance_relative
...
# Full original 16 MiB as one source window; 16 MiB target windows cover the 32 MiB TIP.
SOURCE_WINDOW = ORIGINAL_ROM_SIZE
TARGET_WINDOW = ORIGINAL_ROM_SIZE
assert MAIN_TIP_ROM_SIZE == 2 * ORIGINAL_ROM_SIZE
...
def identity(path: Path, data: bytes | None = None) -> dict[str, Any]:
    ...
    rel = advance_relative(path) if path.resolve().is_relative_to(ROOT.resolve()) else str(path)
    return {
        "path": rel,
        "size": len(payload),
