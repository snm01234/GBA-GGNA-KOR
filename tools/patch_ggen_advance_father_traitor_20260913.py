"""Correct Mineva/?? map line お父さまの……裏切り者.

の was calqued as 의; the line is vocative accusation:
아버님은…… / 배신자야……！！
"""
...
BATCH_ID = "father-traitor-dialogue-20260913"
IDENTITY_KEY = "father_traitor_20260913_sha256"
BATCH_KEY = "father_traitor_20260913"
OUT = ROOT / "outputs" / "20260913_father_traitor"
WORK = OUT / "ggen_father_traitor_20260913.gba"
SNAPSHOT = ROOT / "analysis" / "ggen_advance_translation_merged_20260913_father_traitor.json"
MAIN_SAV = ROOT / "SD Gundam GGeneration Advance (Korean).sav"
CAVE_START = 0x0113552B
CAVE_END = 0x01230000
MAP_BANK = (0x00F00000, 0x00FC0000)
RECORD_ID = "GGA-MAPSCRIPT-00F57A8A"
SOURCE = "お父さまの……\\nうらぎりもの……！！"
OLD_SEGMENTS = ["아버님의……", "배신자……！！"]
NEW_SEGMENTS = ["아버님은……", "배신자야……！！"]
NOTES = "お父さまの……うらぎりもの. 의/명사 오역 → 아버님은…… 배신자야……！！"
...
def main() -> None:
    ...
    gate(all(b == 0 for b in parent[CAVE_START:CAVE_END]), "father-traitor cave is not empty")
    ...
    for char in hangul_chars("아버님은배신자야"):
    ...
    leftover = [
        ...
        if str(item.get("source_text") or "") == SOURCE
        and (
            "아버님의……" in str(item.get("translation_ko") or "")
            or any("아버님의……" in str(seg) for seg in (item.get("translation_segments") or []))
        )
    ]
    gate(not leftover, f"아버님의 leftover {leftover}")
    ...
    gate(cave_cursor <= CAVE_END, "father-traitor cave overflow")
    ...
    shutil.copy2(MAIN_SAV, OUT / "ggen_father_traitor_20260913.sav")
    report = {
        "schema_version": 1,
        "kind": "ggen_advance_father_traitor_20260913",
        "result": "PASS",
        "batch_id": BATCH_ID,
        ...
        "verification": {
            "result": "PASS",
            "father_is_traitor": True,
            "runtime_emulator": "not run; static ROM verification only",
            "changed_bytes": len(changed),
        },
    }
    (OUT / "manifest.json").write_text(json.dumps(report, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    (ROOT / "analysis" / "ggen_advance_father_traitor_candidate_20260913.json").write_text(
