#!/usr/bin/env python3
"""Apply wave-4 bitmap-closed ID-command lines to sheet and current-main ROM."""
...
from ggen_advance_painted_glyph_identity import load_galmuri8, paint_8x16, recover_unique_8x16_slots  # noqa: E402
...
from patch_ggen_advance_intermission_text_consumers_20260904 import ROM_BASE, gate, payload_at, sha256, u32  # noqa: E402

import _tmp_classify_pending_readable_20260907 as decode  # noqa: E402

BATCH_ID = "bitmap-wave4-20260907"
KO_MAP = ROOT / "analysis" / "ggen_advance_8x16_bitmap_wave4_ko_20260907.json"
SNAPSHOT = ROOT / "analysis" / "ggen_advance_translation_merged_20260907_bitmap_wave4.json"
OUT_DIR = ROOT / "outputs" / "20260907_ggen_advance_bitmap_wave4"
OUTPUT = OUT_DIR / "ggen_advance_bitmap_wave4_candidate_20260907.gba"
OUT_SAV = OUT_DIR / "ggen_advance_bitmap_wave4_candidate_20260907.sav"
MAIN_SAV = ROOT / "SD Gundam GGeneration Advance (Korean).sav"
REPORT = ROOT / "analysis" / "ggen_advance_bitmap_wave4_candidate_20260907.json"
MANIFEST = OUT_DIR / "manifest.json"
CAVE_START = 0x012B1780
CAVE_END = 0x012BF000
...
def main() -> int:
    ...
    gate(all(value == 0 for value in current[CAVE_START:CAVE_END]), "wave4 cave is not zero-filled")
    ...
    for row in merged["records"]:
        ...
        if font != "8x16" or text_missing or category != "id_command_name":
            continue
        korean = idcmd_map.get(decoded)
        if korean is None:
            skipped.append({"record_id": row["record_id"], "reason": "held or unmapped", "decoded": decoded})
            continue
        jobs.append({"row": row, "source": decoded, "korean": korean, "notes": "8x16 비트맵 wave4 ID커맨드"})

    hangul8 = set().union(*(prev.hangul_chars(job["korean"]) for job in jobs)) if jobs else set()
    ...
    for job in jobs:
        ...
        prev.mark_row(row, job["source"], job["korean"], job["notes"])
        ...
        if any(payload != orig_raw for payload in current_payloads):
            sheet_only += 1
            continue
        payload = prev.encode_text(job["korean"], slots8, encode8_map)
        key = payload.hex()
        if key in written:
            ...
            gate(end <= CAVE_END, "wave4 cave overflow")
    ...
    identity["bitmap_wave4_batch_identity_sha256"] = new_identity
    ...
    merged["bitmap_wave4_batch_20260907"] = {
        "batch_id": BATCH_ID,
    ...
    report = {
        ...
        "verification": {
            ...
            "held_bitmap_collides_with_frame": maps["policy"]["held_bitmap_collides_with_frame"],
