...
플레이 중에 보이는 글과 글자 그래픽을 한국어로 바꿉니다. 한글 글꼴(8×16 / 12×12, Galmuri)을 넣고, 원본 16 MiB는 유지한 채 추가 16 MiB에 한글 텍스트·글꼴을 둡니다.

| 구분 | 내용 |
|---|---|
| 대사 | 시나리오·이벤트, 전투 대사, ID 커맨드 외침·컷인 |
| 이름·용어 | 기체, 파일럿, 무장, 지명, 계급, 도감 프로필 |
| 메뉴·시스템 | 인터미션, 전투 커맨드, 설정, 도움말, 정렬·보급 등 UI |
| 그래픽 문구 | 스테이지 타이틀, 상황 맵, 크레딧, 배지·버튼·팝업·플레이트 |

식별된 번역 레코드 기준 **translated 19,429**건이 메인 TIP에 반영되어 있습니다. ROM 안의 모든 일본어를 없앤 것은 아니며, 실플레이에서 남은 일본어나 깨진 문구가 있으면 버그로 제보해 주세요.
